...
if(cv_s == 0 || cv_t == 0)  # there's no HiS and/or no HiT, so no correlation and copula not needed
{
  if(cv_s == 0)
	risks <- rep(1, pop_size)
  else
	risks <- rgamma(pop_size, shape=k, scale=1/k)

  if(cv_t == 0)
	FoIs <- rep(1, pop_size)
  else
	FoIs <- rgamma(pop_size, shape=m, scale=1/m)

  risk_FoIs <- cbind(risks, FoIs)
} else
{
...
filename <- paste("risksFoIs_births_his",cv_s,"_hit",cv_t,"_cor",cor_level,"_p",pop_size,"_seed",seed,".txt",sep="")
